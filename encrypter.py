...
def encrypter():
    ...
    key_1 = AESCCM.generate_key(bit_length=128)
    key_2 = AESCCM.generate_key(bit_length=128)
    key_3 = AESCCM.generate_key(bit_length=128)
    ...
    for index in range(0, len(files)):
        if index % 4 == 0:
            ...
            AESCCMAlgo(files[index], key_1, nonce13)
        elif index % 4 == 1:
            """ ChaChaAlgo(files[index], key_2, nonce12) """
            AESCCMAlgo(files[index], key_2, nonce13)
        elif index % 4 == 2:
            """ AESGCMAlgo(files[index], key_3, nonce12) """
            AESCCMAlgo(files[index], key_3, nonce13)
        else:
            AESCCMAlgo(files[index], key_4, nonce13)
    secret_information = (key_1)+b":::::"+(key_2)+b":::::"+(key_3) + \
        b":::::"+(key_4)+b":::::" + (nonce13)  # All the keys

    # Encrypting all the keys with algo1 using key_1
    AESAlgo(secret_information, key_sec)
    public_key = open("./key/Main_Key.pem", "wb")
    public_key.write(key_sec)  # key_1 stored in Main_Key.pem
    public_key.close()
